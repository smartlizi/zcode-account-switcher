...
**Q: 启动后某些账号额度显示「Token 已过期或无效」？**
A: 通常是服务端限流（429）或首次激活延迟导致，工具会自动退避重试。若手动刷新后仍提示「该账号 Token 已过期，请删除后重新登录」，说明该账号 token 已真正失效，需在工具里删除该账号后重新走「添加账号」流程。

**Q: 添加账号时浏览器提示「要打开 electron 吗」？**
A: 这是系统正常的协议确认弹窗（`zcode://` 协议唤起工具），点「打开」即可让工具接收登录回调完成添加。
